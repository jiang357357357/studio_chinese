        ...
        render() {
            return (
                <Button
                    ...
                    title={t(
                        "Copy options as a JSON literal to the clipboard",
                        "Copy options as a JSON literal to the clipboard"
                    )}
                >
                    {t("Copy Options", "Copy Options")}
                </Button>
            );
        }
    }
            ...
            if (column.formatterParams) {
                try {
                    ...
                    messages.push(
                        new Message(
                            MessageType.ERROR,
                            `${t("Invalid JSON", "Invalid JSON")}: ${err}`,
...
class TabulatorOptions extends EezObject {
    ...
    static classInfo: ClassInfo = {
        ...
        properties: [
            ...
            {
                name: "syncColumns",
                displayName: t("Sync columns with structure", "Sync columns with structure"),
                type: PropertyType.Boolean,
                checkboxStyleSwitch: true,
            ...
            {
                name: "syncStructure",
                displayName: t("Structure", "Structure"),
                ...
                arrayPropertyEditorAdditionalButtons: (
                    ...
                    return [
                        <IconAction
                            ...
                            onClick={event => {
                                ...
                                (async () => {
                                    const result = await showGenericDialog({
                                        dialogDefinition: {
                                            title: t("Select Structure", "Select Structure"),
                                            fields: [
                                                {
                                                    name: "structure",
                                                    type: "enum",
                            ...
                            title={t("Build From Structure Definition", "Build From Structure Definition")}
                        />
                    ];
                }
            },
                        ...
                        options.columns.forEach(column => {
                            if (!struct.fieldsMap.get(column.field)) {
                                messages.push(
                                    new Message(
                                        MessageType.ERROR,
                                        `${t("Field", "Field")} "${column.field}" ${t("not found in the structure", "not found in the structure")} "${options.syncStructure}"`,
...
export class TabulatorWidget extends Widget {
    static classInfo = makeDerivedClassInfo(Widget.classInfo, {
        ...
        properties: [
            ...
            {
                name: "options",
                displayName: t("Basic options", "Basic options"),
            ...
            makeExpressionProperty(
                {
                    name: "configuration",
                    displayName: t("Advanced options", "Advanced options"),
                    formText: () => (
                        <span>
                            {t("Advanced options are set via JSON value, check", "Advanced options are set via JSON value, check")} {" "}
                            <a
                                href="#"
                                onClick={event => {
                                    event.preventDefault();
                            ...
                            >
                                {t("Tabulator documentation", "Tabulator documentation")}
                            </a>{" "}
                            {t("for available options.", "for available options.")}
                        </span>
                    ),
                    type: PropertyType.MultilineText,
                    propertyGridGroup: specificGroup
            ...
            makeExpressionProperty(
                {
                    name: "persistance",
                    displayName: t("Persistent configuration", "Persistent configuration"),
                    type: PropertyType.MultilineText,
                    propertyGridGroup: specificGroup
                },
                "json"
