...
registerActionComponents("Network", [
    {
        ...
        properties: [
            ...
            {
                name: "ipAddress",
                displayName: t("IP Address", "IP Address"),
                type: "expression",
        ...
        execute: async (context: IDashboardComponentContext) => {
            ...
            if (!ipAddress || typeof ipAddress != "string") {
                context.throwError(t("invalid IP Address property", "invalid IP Address property"));
                return;
            }

            const port = context.evalProperty<number>("port");
            if (port == undefined || typeof port != "number") {
                context.throwError(t("invalid Port property", "invalid Port property"));
                return;
            }
    ...
    },
    {
        ...
        properties: [
            ...
            },
            {
                ...
                type: "expression",
                valueType: "object:string",
                formText: t("This property is optional", "This property is optional"),
                optional: () => true
            },
            {
                name: "maxConnections",
                displayName: t("Max. Connections", "Max. Connections"),
                type: "expression",
                valueType: "object:number"
            }
        ],
        ...
        execute: async (context: IDashboardComponentContext) => {
            ...
            if (context.getInputValue("@seqin") !== undefined) {
                ...
                if (
                    ...
                    context.throwError(t("invalid Max. connections property", "invalid Max. connections property"));
                    return;
                }
                ...
                if (!executionState) {
                    context.throwError(t("Never started", "Never started"));
                    return;
                }
...
const TCP_SOCKET_EVENTS = [
    { id: "ready", label: t("Ready", "Ready"), paramExpressionType: "null" },
    { id: "data", label: t("Data", "Data"), paramExpressionType: "string" },
    { id: "close", label: t("Close", "Close"), paramExpressionType: "null" },
    { id: "end", label: t("End", "End"), paramExpressionType: "null" },
    { id: "error", label: t("Error", "Error"), paramExpressionType: "string" },
    { id: "timeout", label: t("Timeout", "Timeout"), paramExpressionType: "null" }
];
    ...
    static classInfo: ClassInfo = {
        properties: [
            ...
            },
            {
                ...
                type: PropertyType.Enum,
                enumItems: [
                    { id: "flow", label: t("Flow", "Flow") },
                    { id: "action", label: t("Action", "Action") }
                ],
                enumDisallowUndefined: true,
        ...
        listLabel: (eventHandler: EventHandler, collapsed) =>
            !collapsed
                ? ""
                : `${eventHandler.eventName} ${eventHandler.handlerType}${eventHandler.handlerType == "action"
                    ? `: ${eventHandler.action}`
                    : ""
                }`,
        ...
        newItem: async (eventHandlers: EventHandler[]) => {
            ...
            if (eventEnumItems.length == 0) {
                notification.info(t("All event handlers are already defined", "All event handlers are already defined"));
                return;
            }

            const result = await showGenericDialog({
                dialogDefinition: {
                    title: t("New Event Handler", "New Event Handler"),
                    fields: [
                        {
                            name: "eventName",
                            displayName: t("Event", "Event"),
        ...
        execute: (context: IDashboardComponentContext) => {
            ...
            if (tcpSocket) {
                if (tcpSocket.socket) {
                    ...
                    context.throwError(t("tcp socket is not connected", "tcp socket is not connected"));
                }

                context.propagateValueThroughSeqout();
            } else {
                context.throwError(t("tcp socket not found", "tcp socket not found"));
            }
        }
    });
...
registerActionComponents("Network", [
    {
        ...
        execute: (context: IDashboardComponentContext) => {
            ...
            if (data == undefined || typeof data != "string") {
                context.throwError(t("invalid Data property", "invalid Data property"));
                return;
            }
