                    ...
                    {flagNames.map(flagName => {
                        ...
                        return (
                            <Checkbox
                                key={flagName}
                                state={state}
                                label={t(flagName, humanize(flagName))}
                    ...
                    {stateNames.map(stateName => {
                        ...
                        return (
                            <Checkbox
                                key={stateName}
                                state={state}
                                label={t(stateName, humanize(stateName))}
                                onChange={(value: boolean) => {
                                    this.context.undoManager.setCombineCommands(
                                        true
                                    );
    ...
    static classInfo = makeDerivedClassInfo(Widget.classInfo, {
        ...
        properties: [
            ...
            },
            {
                ...
                displayName: t("Scroll direction", "Scroll direction"),
                type: PropertyType.Enum,
                enumItems: [
                    {
                        id: "none",
            ...
            {
                name: "scrollSnapX",
                displayName: t("Scroll snap X", "Scroll snap X"),
                type: PropertyType.Enum,
                enumItems: [
                    {
                        id: "none",
            ...
            {
                name: "scrollSnapY",
                displayName: t("Scroll snap Y", "Scroll snap Y"),
                type: PropertyType.Enum,
                enumItems: [
                    {
                        id: "none",
            ...
            ...makeLvglExpressionProperty(
                ...
                ["literal", "expression"],
                {
                    displayName: t("Disabled", "Disabled"),
                    propertyGridGroup: statesGroup
                }
            ),
            {
                ...
                propertyMenu: (props: PropertyProps): Electron.MenuItem[] => {
                    ...
                    if (props.objects.length === 1) {
                        ...
                        if (widget.localStyles.hasModifications) {
                            menuItems.push(
                                new MenuItem({
                                    label: t("Reset All Modifications", "Reset All Modifications"),
                                    click: () => {
                            ...
                            menuItems.push(
                                new MenuItem({
                                    label: t("Create New Style", "Create New Style"),
                                    click: async () => {
                                        const projectStore =
                ...
                if (eventHandler.eventName == "CHECKED") {
                    build.line(
                        `lv_obj_add_event_cb(obj, ${build.getCheckedEventHandlerCallbackName(
                            this
                        )}, LV_EVENT_VALUE_CHANGED, (void *)${eventHandler.userData
                        });`
                    );
                    ...
                    build.line(
                        `lv_obj_add_event_cb(obj, ${build.getActionFunctionName(
                            eventHandler.action
                        )}, LV_EVENT_${eventHandler.eventName}, (void *)${eventHandler.userData
                        });`
                    );
                }
            }
